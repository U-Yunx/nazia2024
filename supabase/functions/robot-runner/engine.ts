...
export interface Position {
  ...
  /**
   * Set by the peak-return rule once a position gave back
   * `risk.peakReturnGivebackPct` from its best unrealized PnL ("it dipped").
   * While set, the peak is frozen at the LAST highest profit and markToMarket
   * closes the trade the moment PnL returns to it (reason 'peak_return').
   * Mirrors the client engine so browser and server behave the same.
   */
...
export interface RiskConfig {
   ...
   * — "watch it dip, then bank it the moment it returns to the top".
   * Mirrors the client engine. Off by default.
   */
  peakReturnClose: boolean
  /**
   * How much a position must give back from its peak unrealized PnL (as a %)
   * before the peak-return close arms. 10 = dipped 10% off its best profit
   * before a return to the peak counts as a close signal. Guards against
   * closing on sub-pip noise right at the high.
   */
  ...
  /**
   * Auto-reverse ("reverse open position trading") — mirrors the client
   * engine. When activated, the robot closes a strategy-tagged position once
   * it is down `reverseTriggerPips` from entry and immediately opens the
   * OPPOSITE direction at the same size, automatically, while the robot runs.
   * Never touches manual positions; the reversed open passes every risk gate.
   */
  autoReverse?: boolean
...
/** Default adverse pips from entry that triggers an auto-reverse (mirrors the
 *  client engine's DEFAULT_REVERSE_TRIGGER_PIPS). */
const DEFAULT_REVERSE_TRIGGER_PIPS = 15;
...
/** Track each position's best unrealized PnL for the profit-pullback lock and
 * the peak-return close. Only runs while either rule is on; returns the same
 * state reference when nothing moved, so an idling account never churns
 * persistence writes. */
function trackProfitPeaks(state: AccountState, rates: RatesMap): AccountState {
  ...
  const positions = state.positions.map((p) => {
    ...
    // Peak-return arm: once the position gave back `peakReturnGivebackPct`
    ...
    // only engages after profit passed the $1 activation floor.
    ...
    // rule is on; if the user turned the rule off, tracking resumes.
...
/**
 * Auto-reverse ("flip a loser") — the Deno mirror of the client rule.
 *
 ...
 * risk:reward — riding the reversal instead of waiting for the stop. Runs
 * automatically on every mark-to-market pass, so the background runner
 * behaves exactly like the browser. Manual trades are never touched; the
 * reversed open still passes every risk gate (a blocked open just flattens
 * the loser).
 */
export function autoReversePositions(
  ...
  for (const p of [...next.positions]) {
    ...
    // Same risk geometry as the original leg: same stop distance from entry,
    // same risk:reward multiple, same size — just flipped.
...
export function markToMarket(
  ...
  // Profit-pullback lock / peak-return — record peaks first so give-back is
  // measured this pass.
  ...
  for (const p of next.positions) {
    ...
    // trackProfitPeaks; once armed the peak is frozen, so a return to it is
    // unambiguous. Mirrors the client engine.
  ...
  // Auto-reverse ("reverse open position trading") — after the regular exits
  // have had their say, a robot position still bleeding past the trigger pips
  // is closed and flipped into the opposite direction at the same size,
  // automatically, while the robot runs. Runs AFTER the normal rules so a
  // legit stop / target / pull-back / drawdown close always wins.
      ...
      positions: state.positions.map((p) =>
        p.id === position.id
          ? {
              ...
              partialTaken: true,
              // Fresh leg — reset the pullback peak, peak-return arm and
              // price-mark history so rules measure from the split.
              peakProfitUsd: 0,
              peakRetraced: false,
